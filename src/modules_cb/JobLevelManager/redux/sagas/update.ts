...
export function* updateJobLevelAsync(action: UpdateJobLevelAction) {
  try {
    ...
    if (rs.code !== 0) {
      NotificationError('Cập nhật job level không thành công', "Lỗi: " + rs.message)
    } else {
      NotificationSuccess('Thành công', "Cập nhật job level thành công");
    ...
    NotificationError('Cập nhật job level không thành công', "Lỗi: " + e.message);
  }
}
